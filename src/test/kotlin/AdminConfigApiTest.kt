...
class AdminConfigApiTest {

    @Test
    fun `admin can list and delete all saved configs`() = testApplication {
        environment {
        ...
        client.post("/api/auth/register") {
            ...
            setBody(RegisterRequest(phone = "+79996660001", nickname = "admin", password = "strongpass123", telegramId = null))
        }
        val adminUser = userRepository.findByPhone("+79996660001")!!
        userRepository.updateApproval(adminUser.id, true)
        userRepository.updateIsAdmin(adminUser.id, true)

        client.post("/api/auth/register") {
            contentType(ContentType.Application.Json)
            setBody(RegisterRequest(phone = "+79996660002", nickname = "user", password = "strongpass123", telegramId = null))
        }
        val regularUser = userRepository.findByPhone("+79996660002")!!
        userRepository.updateApproval(regularUser.id, true)

        val loginResponse = client.post("/api/auth/login") {
            contentType(ContentType.Application.Json)
            setBody(LoginRequest(phone = "+79996660001", password = "strongpass123"))
        }
        ...
        val device = deviceRepository.createDevice(regularUser.id, "Pixel")
        val savedConfig = deviceServerRepository.upsert(device.id, server.id, testAwgConfig())

        val listResponse = client.get("/api/configs") {
        ...
        assertEquals(
            listOf(
                AdminDeviceConfigResponse(
                    ...
                    userNickname = regularUser.nickname,
                    deviceId = device.id,
        ...
        val deleteResponse = client.delete("/api/configs/${savedConfig.id}") {
            header(HttpHeaders.Authorization, "Bearer $token")
        }

        assertEquals(HttpStatusCode.NoContent, deleteResponse.status)
        assertTrue(deviceServerRepository.findByConfigId(savedConfig.id) == null)
        assertEquals(1, cleanupService.cleanedConfigs.size)
        assertEquals(savedConfig.id, cleanupService.cleanedConfigs.single().config.id)
        assertEquals(server.id, cleanupService.cleanedConfigs.single().server.id)
    }

    private fun testConfig(): AppConfig {
        val tempDir = createTempDirectory("admin-config-api-test")
        return AppConfig(
